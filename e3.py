...
def line_follow():
	while 1:
		...
		if detected_green[0] or detected_green[1]:
			...
			if value_interpreter.over_max_left > 0 and (value[3] > THRESHOLD or value[4] > THRESHOLD) and value_interpreter.over_max_left > value_interpreter.over_max_right: # left sharp turn
				driver.stop()
				driver.reset()
				driver.lr(-SHARP_TURN, SHARP_TURN, TURNING_SPEED)
				driver.block()
				driver.reset()
				driver.lr(1, 1, FORWARD_SPEED)
				driver.block()
				value = lsa.values()
			elif value_interpreter.over_max_right > 0 and (value[3] > THRESHOLD or value[4] > THRESHOLD) and value_interpreter.over_max_right > value_interpreter.over_max_left: # right sharp turn
				driver.stop()
				driver.reset()		
				driver.lr(SHARP_TURN, -SHARP_TURN, TURNING_SPEED)
				driver.block()
